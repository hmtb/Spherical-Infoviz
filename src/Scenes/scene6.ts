...
export class Scene6 extends SceneObject {
    instant = false;
    year: any;
    ...
    constructor(window: allofw.OpenGLWindow, omni: allofw.IOmniStereo, startTime: number,size:number) {
        ...
        this.currentText = [];
        var data = require("d3").csv.parse(require("fs").readFileSync("studyData/data/scene6.csv", "utf-8"));
         ...
         this.cubeSpec = Stardust.mark.compile(`
            ...
            mark Mark(lon: float, lat: float, val: float,r:float,g:float,b:float) {
                ...
                let radius = 0.04;
                let color = Color(r/255, g/255, b/255, 1);
